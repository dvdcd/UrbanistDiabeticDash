...
bool ConfigStore::load(DashConfig &out) {
  ...
  out.show_clock        = prefs.getBool(  "show_clk",   true);
  out.pulse_speed       = prefs.getInt(   "pulse_spd",  1);
  out.pulse_min         = prefs.getInt(   "pulse_min",  50);
  out.wave_speed        = prefs.getInt(   "wave_spd",   0);
  out.status_bar_style  = prefs.getInt(   "bar_style",  2);
  out.show_sparkline    = prefs.getBool(  "show_spark", true);
  out.sparkline_auto    = prefs.getBool(  "spark_auto", false);
  out.sparkline_shadow  = prefs.getBool(  "sp_shad",    false);
  out.sparkline_fill    = prefs.getBool(  "sp_fill",    false);
  ...
  out.show_stars        = prefs.getBool(  "stars",      false);
  ...
  out.color_wave2       = prefs.getUInt(  "col_wave2",  0x0a0020);
  out.color_boat_hull   = prefs.getUInt(  "col_bthul",  0xC87828);
  out.color_boat_sail   = prefs.getUInt(  "col_btsal",  0xAA8C3C);
...
void ConfigStore::save(const DashConfig &c) {
  ...
  prefs.putBool(  "show_clk",    c.show_clock);
  ...
  prefs.putBool(  "sp_shad",     c.sparkline_shadow);
  prefs.putBool(  "sp_fill",     c.sparkline_fill);
  ...
  prefs.putUInt(  "col_wave2",   c.color_wave2);
  prefs.putUInt(  "col_bthul",   c.color_boat_hull);
  prefs.putUInt(  "col_btsal",   c.color_boat_sail);
